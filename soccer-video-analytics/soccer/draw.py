...
class Draw:
    ...
    def draw_bounding_box(
        ...
        # Tracker/camera compensation can occasionally flip corner ordering.
        x0f = float(min(rectangle[0][0], rectangle[1][0]))
        y0f = float(min(rectangle[0][1], rectangle[1][1]))
        x1f = float(max(rectangle[0][0], rectangle[1][0]))
        y1f = float(max(rectangle[0][1], rectangle[1][1]))

        # Skip invalid coordinates that occasionally appear on unstable tracks.
        if not np.isfinite([x0f, y0f, x1f, y1f]).all():
            return img
        ...
        draw = PIL.ImageDraw.Draw(img)
        # Keep radius valid for tiny boxes to avoid PIL geometry errors.
        radius = min(7, (x1 - x0) // 2, (y1 - y0) // 2)
        if radius <= 0:
        ...
        try:
            ...
            # Fallback for Pillow edge cases with tiny/degenerate geometry.
